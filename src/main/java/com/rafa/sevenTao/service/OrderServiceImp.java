...
public class OrderServiceImp implements OrderService {
    ...
    private int countByDay(Room room, Date date) {
        ...
        RoomPrice roomPrice = room.getRoomPrices().stream()
                .filter(rp -> rp.getWeekDay() == weekDay)
    ...
    public int countPrice(Room room, Date checkInDate, Date checkOutDate) {
        ...
        int totalPrice = 0;
        Function<Date, Integer> getPrice =
                d -> Integer.min(countByDay(room, d), countBySpecialDate(room, d));
        if (livingDate != null) {
            List<Integer> priceList = livingDate.parallelStream().map(getPrice).toList();
            totalPrice = priceList.parallelStream().mapToInt(p->p).sum();
        }
        return totalPrice;
    }
}
